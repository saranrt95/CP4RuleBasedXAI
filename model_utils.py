...
def extract_and_save_rules(model, res_path, rulesetfile_full, data_tr,  output = "output", save = False):
    y_train=data_tr[output]
    data_features = data_tr.drop([output],axis=1)
    #data_tr.drop(["output"],axis=1,inplace=True)
    # rule extraction
    rules, changeclsidx = ExtractRulesFromDT(model, list(model.feature_names_in_), [0,1], output)
    if save:
        ...
        for rule in rules:
            ...
            with open(f"{res_path}/{rulesetfile_full}","a") as rulefile:
                ...
                rulefile.write(str(rule)+f", COVERING: {covering}, ERROR: {error}\n")
    # Rule thresholds
